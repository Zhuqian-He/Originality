...
# Eighth Step: New Words Combination
...
HISTORICAL_NEW_COMBS_PATH = "Arts_results/new_word_combs.csv"  
Y2024_WORDS_PATH = "Data/Output/papers_words.csv"  
OUTPUT_PATH = "Data/Output/new_word_combs.csv"  
...
logging.basicConfig(
    ...
    datefmt='%Y-%m-%d %H:%M:%S',
    handlers=[
        logging.FileHandler('Log/8_New_word_comb.log'),
...
def process_chunk(chunk: pd.DataFrame) -> Set[Tuple[str, str]]:
    ...
    for idx, row in chunk.iterrows():
        ...
        all_words = []
        if pd.notna(title):
            all_words.extend([w.strip() for w in word_splitter.split(title)])
        if pd.notna(abstract):
            all_words.extend([w.strip() for w in word_splitter.split(abstract)])
        paper_words = frozenset([w for w in all_words if w])
        for comb in it.combinations(paper_words, 2):
...
if __name__ == "__main__":
    ...
    # --------------------------
    logging.info("Loading historical word combinations up to 2023...")
    ...
    try:
        ...
        logging.error(f"Failed to get total rows: {e}")
        total_historical_papers = None

    df_chunks = pd.read_csv(
        HISTORICAL_WORDS_PATH,
        encoding='utf-8',
        usecols=[1, 2],  
        skiprows=1,  
        dtype=str,  
    ...
    with multiprocessing.Pool(processes=PROCESS_NUM, maxtasksperchild=1) as pool:
        for chunk_combs in tqdm(
            ...
            total=total_chunks,
            desc="Processing historical word combinations"
        ):
    ...
    with open(HISTORICAL_WORDS_PATH, 'r', encoding='utf-8') as f:
        ...
        for line in tqdm(reader, total=total_historical_papers, desc="Processing historical word combinations"):
            title_words = [w.strip() for w in line[1].split() if w.strip()]
            abstract_words = [w.strip() for w in line[2].split() if w.strip()]
            paper_words = set(title_words + abstract_words)
        
            combs = list(it.combinations(paper_words, 2))
    ...
    # --------------------------
    # 2. Load new_words_comb data up to 2023
    # --------------------------
    logging.info("\nLoading historical new_word_combs.csv up to 2023...")
    ...
    with open(HISTORICAL_NEW_COMBS_PATH, 'r', encoding='utf-8') as f:
        ...
        for line in reader:
            word1, word2, first_paper_id, reuse = line
            comb = tuple(sorted([word1, word2]))
            historical_new_combs[comb] = (first_paper_id, int(reuse))
            row_count += 1

    logging.info(f"Total rows in historical new_word_combs.csv: {row_count}")
    ...
    logging.info("\nProcessing 2024 paper word combinations...")
    y2024_papers = []
    ...
    for pub_date, paper_id, paper_words in tqdm(
        y2024_papers_sorted, 
    ...
    # --------------------------
    # 4. Update new_word_comb data
    # --------------------------
    logging.info("\nUpdating new_word_combs.csv...")
    updated_new_combs = {}
    ...
    with open(OUTPUT_PATH, 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Word1', 'Word2', 'PaperID', 'Reuse'])  
        total_output_rows += 1
    
        for comb in sorted(updated_new_combs.keys()):
            word1, word2 = comb
            first_id, reuse = updated_new_combs[comb]
            writer.writerow([word1, word2, first_id, reuse])
            total_output_rows += 1
